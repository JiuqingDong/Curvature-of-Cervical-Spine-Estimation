...
# python main.py --data_dir dataPath --dataset spinal  --phase eval --resume model_29.pth
def collater(data):
    out_data_dict = {}
    for name in data[0]:
        out_data_dict[name] = []
...
class Network(object):
    def __init__(self, args):
        torch.manual_seed(317)  # 生成的随机数是确定的 317 267 268 257-no # 198还可以 199-196-no 269-no
        ...
        #self.model = self.load_model(model=self.model, resume=self.resume, strict=True)
        self.num_classes = args.num_classes
        self.decoder = decoder.DecDecoder(K=args.K, conf_thresh=args.conf_thresh)  # k=17
    ...
    def load_model(self, model, resume, strict=True):
        ...
        print('loaded weights from {}, epoch {}'.format(resume, checkpoint['epoch']))  # resume可以在原来的基础上继续训练？
    ...
    def train_network(self, args):
        ...
        #events_log = SummaryWriter(log_dir='./weights_spinal/events_log')
        ...
        # scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, 12, gamma=0.96, last_epoch=-1)
        if args.ngpus>0:
        ...
        for epoch in range(1, args.num_epoch+1):
            ...
            epoch_loss, epoch_iter_loss = self.run_epoch(phase='train', data_loader=dsets_loader['train'], criterion=criterion)
            train_loss.append(epoch_loss)
            train_iter_loss.append(epoch_iter_loss)
            scheduler.step(epoch)

            epoch_loss,  epoch_iter_loss= self.run_epoch(phase='val', data_loader=dsets_loader['val'], criterion=criterion)
            val_loss.append(epoch_loss)
            val_iter_loss.append(epoch_iter_loss)
        ...
        for data_dict in data_loader:
            ...
            if phase == 'train':
                ...
                with torch.enable_grad():
                    pr_decs_en, pr_decs, dec_dict_cat, dec_dict_de_c4, dec_dict_de_c3 = self.model(data_dict['input'])
                    loss = criterion(pr_decs_en, pr_decs,dec_dict_cat, dec_dict_de_c4, dec_dict_de_c3, data_dict)
                    loss.backward()
                    self.optimizer.step()
            else:
                with torch.no_grad():
                    pr_decs_en, pr_decs,dec_dict_cat, dec_dict_de_c4, dec_dict_de_c3 = self.model(data_dict['input'])
                    loss = criterion(pr_decs_en, pr_decs, dec_dict_cat, dec_dict_de_c4, dec_dict_de_c3, data_dict)
            assert isinstance(loss, object)

            loss_ = loss.data.cpu().numpy()
            running_loss_save.append(loss_)

            running_loss += loss.item()

        epoch_loss = running_loss / len(data_loader)
        print('{} loss: {}'.format(phase, epoch_loss))
        return epoch_loss, running_loss_save
