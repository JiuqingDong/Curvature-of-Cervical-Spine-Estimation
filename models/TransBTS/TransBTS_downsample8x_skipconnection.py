...
class TransformerBTS(nn.Module):
    ...
    def forward(self, x, auxillary_output_layers=[1, 2, 3, 4]):
        ...
        x1, x2, x3, x4, encoder_output, intmd_encoder_outputs = self.encode(x)
        feat_encode = []
        feat_encode.append(x1)
        feat_encode.append(x2)
        feat_encode.append(x3)
        feat_encode.append(x4)
        d1_1, d1_2, d1_3, d1_4, d1_5 = self.decode(x1, x2, x3, x4, encoder_output, intmd_encoder_outputs, auxillary_output_layers)
        feat_encode.append(d1_5)
        feat_decode=[]
        feat_decode.append(d1_1)
        feat_decode.append(d1_2)
        feat_decode.append(d1_3)
        feat_decode.append(d1_4)
        feat_decode.append(d1_5)
        # print("feat_encode:", np.array(feat_encode)[-1].shape)  # [2, 512, 32, 16]
        # print("feat_decode:", np.array(feat_decode)[-1].shape)  # [2, 512, 32, 16]

        #for i in range(len(feat_decode)):
        #    print("feat_decode[{}].size(): {}".format(i, feat_decode[i].size()))
...
class BTS(TransformerBTS):
    ...
    def decode(self, x1, x2, x3, x4, x, intmd_x, intmd_layers=[1, 2, 3, 4]):
        ...
        y4 = self.DeUp4(x8, x4)  # (1, 512, 32, 16)
        y4 = self.DeBlock4(y4)
        y3 = self.DeUp3(y4, x3)  # (1, 256, 64, 32)
        y3 = self.DeBlock3(y3)
        y2 = self.DeUp2(y3, x2)  # (1, 128, 128, 64)
        y2 = self.DeBlock2(y2)
        #y1 = self.DeUp1(y2, x1)  # (1, 64, 256, 128)
        #y1 = self.DeBlock1(y1)

        return y2, y2, y3, y4, x8
